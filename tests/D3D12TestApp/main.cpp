...
//
// Usage: D3D12TestApp.exe [--frames N] [--hang [--hang-after N]]
...
float4 PSMain(PSIn input) : SV_TARGET { return input.color * Pattern.Sample(PatternSampler, input.uv); }
)";
...
void CSHang(uint3 id : SV_DispatchThreadID)
{
    ...
    void Init(HWND hwnd)
    {
        ...
        // Graphics root signature: CBV b0 (vertex), descriptor table SRV t0 (pixel), static sampler s0.
        ...
        ComPtr<ID3DBlob> ps = CompileShader(kShader, "PSMain", "ps_5_0");
        ...
        g_computeOutput = CreateBuffer(kComputeElements * sizeof(float), D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
    ...
    void RunCompute(UINT frame, bool hang)
    {
        ...
        g_computeList->Dispatch(kComputeElements / 64, 1, 1);
