...
internal static class TimingSubmissionReferences
{
    ...
    internal static long Parse(string reference, string handle, int generation)
    {
        ...
        if (parts.Length != 4 || parts[0] != "submission" ||
            ...
            throw new PixToolException("invalid_reference", "submissionRef is not a recorded submission reference.");
        if (parts[1] != handle || version != generation)
            throw new PixToolException("result_expired", "This submission reference belongs to another or changed timing capture. Repeat pix_timing_submissions.",
