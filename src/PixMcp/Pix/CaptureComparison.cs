...
public static class CaptureComparison
{
    public static ComparisonResultDto Compare(ComparisonSnapshot baseline, ComparisonSnapshot candidate,
        QueuePair[]? queuePairs = null, EventPair[]? eventPairs = null)
    {
        ...
        foreach (var (a, b) in queueMatches)
        {
            ...
            foreach (var ga in a.Events.Where(e => !matchedA.Contains(e.EventRef)).GroupBy(Key))
            {
                ...
                var remainingA = left.Where(e => !matchedA.Contains(e.EventRef)).Select(e => e.EventRef).ToArray();
                var remainingB = gb.Where(e => !matchedB.Contains(e.EventRef)).Select(e => e.EventRef).ToArray();
                if (remainingA.Length > 0 && remainingB.Length > 0)
                    ambiguities.Add(new("events", ga.Key, remainingA, remainingB));
            }
        }
        ...
        return new(baseline.Handle, candidate.Handle, paired.Count,
            ...
            baseline.Queues.SelectMany(q => q.Events).Where(e => !matchedA.Contains(e.EventRef) && !ambiguousA.Contains(e.EventRef)).Select(e => e.EventRef).ToArray(),
            candidate.Queues.SelectMany(q => q.Events).Where(e => !matchedB.Contains(e.EventRef) && !ambiguousB.Contains(e.EventRef)).Select(e => e.EventRef).ToArray(),
    ...
    { "id", "apiObjectId", "heapId", "address", "gpuVirtualAddress", "eventRef", "resourceRef", "shaderRef", "markerPath", "event", "index",
      "bindingCount", "bindingOffset", "bindingCountReturned", "nextBindingOffset", "bindingsTruncated", "bindings", "nextCalls" };

    public static JsonElement Normalize(object value)
    {
        JsonNode? Visit(JsonNode? node, bool preserveNames = false) => node switch
        {
            JsonObject obj => new JsonObject(obj.Where(p => preserveNames || !IdentityFields.Contains(p.Key)).OrderBy(p => p.Key, StringComparer.Ordinal)
                .Select(p => KeyValuePair.Create(p.Key, Visit(p.Value, preserveNames || p.Key.Equals("defines", StringComparison.OrdinalIgnoreCase))))),
            JsonArray array => new JsonArray(array.Select(child => Visit(child, preserveNames)).ToArray()),
            _ => node?.DeepClone(),
        };
        return JsonSerializer.SerializeToElement(Visit(JsonSerializer.SerializeToNode(value, Json.Options)), Json.Options);
    }

    public static JsonElement[] NormalizeSet(IEnumerable<object> values)
        => values.Select(Normalize).OrderBy(value => value.GetRawText(), StringComparer.Ordinal).ToArray();
