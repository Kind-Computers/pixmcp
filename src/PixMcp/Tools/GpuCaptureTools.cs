...
public static class GpuCaptureTools
{
    ...
    public static Task<string> Open(PixSession session, [Description("Path to the .wpix GPU capture file.")] string path)
    ...
    public static Task<string> GetInfo(PixSession session, [Description("GPU capture handle")] string handle)
        => Tools.Run(session, "pix_gpu_info", () => Info(session.Get<GpuCaptureHandle>(handle)));
    ...
    public static Task<string> Queues(PixSession session, [Description("GPU capture handle")] string handle)
        => Tools.Run(session, "pix_gpu_queues", () => session.Get<GpuCaptureHandle>(handle).Queues.Select(q => q.ToDto()).ToArray());
    ...
    public static Task<string> Events(
        ...
        [Description("Only direct children of this event index (same queue).")] uint? parentIndex = null,
    ...
    [McpServerTool(Name = "pix_gpu_screenshot"), Description("Saves the screenshot embedded in the GPU capture as a PNG file (and optionally returns it inline as an image).")]
