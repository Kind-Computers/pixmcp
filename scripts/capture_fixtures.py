...
def generate(client, app, output_dir, timing_seconds=3, startup_delay_ms=1500, sleep=time.sleep, only="all"):
    ...
    def capture_gpu(variant):
        primary_error = None
        try:
            pid = launch(f"--hidden --frames 1800 --variant {variant} --startup-delay-ms {startup_delay_ms}")
...
def main():
    ...
    try:
        ...
        report = generate(client, args.app, output_dir, args.timing_seconds, args.startup_delay_ms, only=args.only)
    except Exception as error:
